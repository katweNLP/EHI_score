...
def sigmoid_ehi(PH, EF, NH, OF, LF):
    """
    Computes EHI using the sigmoid-based scaling function.
    Ensures values stay between 0 and 1.
    """
    x = PH + EF - (NH + OF + LF)  # Net positive vs negative hallucination score
...
def Compute_EHI(input_text: str, reference_text: str, generated_text: str) -> Tuple[float, float, float, float, float, float,float]:
    ...
    EHI1 = sigmoid_ehi(PH, EF, NH, OF, LF)
...
def compute_and_save_metrics(data: pd.DataFrame):
    ...
    required_columns = {'InputText', 'ReferenceSummary'}
    models = ['google/pegasus-xsum', 't5-large', 'gpt-3.5-turbo', 'facebook/bart-large-cnn', 'RefSum']
    ...
    # Filter models that are present in the DataFrame
    available_models = [model for model in models if model in data.columns]

    if not available_models:
        st.error("None of the expected model columns are found in the data.")
        return
    ...
    for index, row in data.iterrows():
        ...
        for model in available_models:
            ...
            try:
                ...
                st.warning(f"Error computing metrics for index {index}, model {model}: {e}")
    ...
    if not results:
        st.error("No valid metrics were computed. Check data format.")
        return
    ...
    averages_df = results_df.groupby('Model').mean(numeric_only=True).reset_index()
...
def process_json(data: Union[dict, List[dict]]):
    ...
    for item in data:
        ...
        reference_text = item['ReferenceSummary']
        model_metrics = {}
        
        # Compute metrics for each model separately
        for model in ['facebook/bart-large-cnn', 'google/pegasus-xsum', 't5-large', 'gpt-3.5-turbo']:
    ...
    return results
def extract_refsum_metrics(result_df):
    # Filter results for RefSum only
    refsum_df = result_df[result_df['Model'] == 'RefSum']

    # Convert filtered DataFrame to JSON format
    refsum_json = refsum_df.to_json(orient="records", indent=4)

    # Provide a download button for RefSum JSON results
    st.download_button(
        "Download RefSum Metrics as JSON",
        refsum_json.encode('utf-8'),
        "RefSum_metrics.json",
        "application/json"
    )

    # Compute average for each metric in RefSum
    avg_metrics = refsum_df.drop(columns=["Id", "Model"]).mean().reset_index()
    avg_metrics.columns = ["Metric", "Average Value"]

    # Display the averages as a table
    st.write("Average Metrics for RefSum:")
    st.dataframe(avg_metrics)

    # Convert the average metrics to JSON format
    avg_metrics_json = avg_metrics.to_json(orient="records", indent=4)

    # Provide a download button for the averages JSON
    st.download_button(
        "Download RefSum Average Metrics as JSON",
        avg_metrics_json.encode('utf-8'),
        "RefSum_avg_metrics.json",
        "application/json"
    )
...
def main():
    ...
    if uploaded_file is not None:
        if uploaded_file.name.endswith('.csv'):
            ...
            reference_texts = df['ReferenceSummary'].tolist()
    
            # Loop through each model and compute metrics for each row individually
            models = ['facebook/bart-large-cnn', 'google/pegasus-xsum', 't5-large', 'gpt-3.5-turbo','RefSum']
            ...
            for _, row in df.iterrows():
                ...
                for model in models:
                    generated_text = row[model]  # Get the summary for the current model
                    ...
                    results.append({
                        'Id': row['Id'],  # Assuming the 'Id' column exists in your CSV
                        'Model': model,
                        'EHI': EHI,
                        'Extractiveness Factor (EF)': EF,
                        'Positive Hallucination (PH)': PH,
            ...
            # Convert results DataFrame to JSON format
            results_json = result_df.to_json(orient="records", indent=4)

            # Provide a download button for JSON results
            st.download_button(
            ...
            extract_refsum_metrics(result_df)
            ...
            # Compute and save metrics for model-wise averages
            compute_and_save_metrics(result_df)
    
        elif uploaded_file.name.endswith('.json'):  # This line will only execute if uploaded_file is NOT None
